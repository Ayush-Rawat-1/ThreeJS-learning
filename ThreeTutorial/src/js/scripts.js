...
const plane2Materail=new THREE.MeshBasicMaterial({
    color: 0xffffff,
    wireframe: true,
});
const plane2=new THREE.Mesh(plane2Geometry,plane2Materail);
scene.add(plane2);
...
assetsLoader.load(monkeyUrl.href, (gltf) => {
    console.log(gltf);
    const model = gltf.scene;
    console.log(model.children[0].material.opacity);
    model.children[0].material.opacity=4;
    scene.add(model);
...
function animate(time){
    ...
    for(let i of intersects){
        if(i.object.id === sphere.id){
            i.object.material.color.set(0xff0000);
        }
        if(i.object.name === "The Box"){
            i.object.rotation.x=time/1000;
            i.object.rotation.y=time/1000;
        }
    }
